function connectivityStruct = seedConnectivity(seed,data1,data2,weightFile,toi,AFNI,prefix,sampRate,offset,stats,varargin)
...
seedPhase1 = zeros(length(data1.trial),length(timeWin(1)-20:timeWin(2)+20));% padding with 20 samples
for i = 1:length(data1.trial)
    ...
    seedPhase1(i,:) = phase(hilbert(seedVS1(timeWin(1)-20:timeWin(2)+20)));
end

seedPhase2 = zeros(length(data2.trial),length(timeWin(1)-20:timeWin(2)+20));
for i = 1:length(data2.trial)
    ...
    seedPhase2(i,:) = phase(hilbert(seedVS2(timeWin(1)-20:timeWin(2)+20)));
end
...
for j = 1:length(ctxind)
    ...
    bandPhase1 = zeros(length(data1.trial),length((timeWin(1)-20:timeWin(2)+20))); % padding with 20 samples
    ...
    for l = 1:length(data1.trial) 
        ...
        bandPhase1(l,:) = phase(hilbert(VS1(timeWin(1)-20:timeWin(2)+20)));
        dPhaseCor(l,:) = (seedPhase1(l,21:length(seedPhase1)-20)-bandPhase1(l,21:length(seedPhase1)-20)); % clear padding 
    ...
    bandPhase2 = zeros(length(data2.trial),length(timeWin(1)-20:timeWin(2)+20));
    ...
    for l = 1:length(data2.trial) 
        ...
        bandPhase2(l,:) = phase(hilbert(VS2(timeWin(1)-20:timeWin(2)+20)));
        dPhaseMis(l,:) = (seedPhase2(l,21:length(seedPhase2)-20)-bandPhase2(l,21:length(seedPhase2)-20)); % clear padding 
