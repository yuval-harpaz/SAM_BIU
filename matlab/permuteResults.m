...
for thri=1:length(pThr)
    ...
    if doSize
        ...
        [~,s]=unix(['~/abin/3dcalc -prefix resultsSize',strP,' -a resultsSize+tlrc[1] -exp "ispositive(a)*',num2str(thri),'"']);
    ...
    if doT
        if thri==1;
            ...
            [~,t]=unix('~/abin/3dcalc -prefix resultsT -a resultsT+tlrc[1] -exp "ispositive(a)*"');
