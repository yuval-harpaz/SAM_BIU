function virtualsensorExp(fname,Xlength)
...
% 10s in one screen.
% fname is the name of VS files without the number.
% run as: virtualsensorExp('Global,VS',10)
for i=1:8;if exist([fname,num2str(i)],'file');nvs=i;end;end;
...
switch(nvs)
    case 1
        [x, y1] = textread([fname '1'], '%f %f');
        p = plot(x,y1,'LineWidth',1.5);
    case 2
        [x, y1] = textread([fname '1'], '%f %f');
        [x, y2] = textread([fname '2'], '%f %f');
        y1 = y1 + qspace;
        y2 = y2 - qspace;
